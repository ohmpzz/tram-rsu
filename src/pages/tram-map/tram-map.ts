...
export class TramMapPage {
  ...
  constructor(
    ...
    mapboxgl.accessToken =
      'pk.eyJ1Ijoib2htcHp6IiwiYSI6ImNqZ3M2M3R3bzBmOGoyem1vNHdsYmlyaHgifQ.M5Fs1sHobCWFehNliIqbrw';
  ...
  initTram() {
    this.tramService
      ...
      .pipe(
        take(1),
        map(trams => trams.filter(tram => this.allowedTram(tram))),
        tap(trams =>
          trams.map(tram => {
  ...
  getNextStopDistanceAndDuration(nextStop, tram) {
    return new Promise(resolve => {
      this.tramService
        .getTramStopById(nextStop)
        .subscribe((tramStop: TramStop) => {
          this.tramService
            .getDirection(
              tram.geolocation.geometry.coordinates,
              tramStop.geometry.coordinates
            )
            .subscribe((geo: any) => {
              resolve({
  ...
  async findDistanceAndDuration(tram, currentStop: TramStop) {
    ...
    let nextSeq = tram.next.substring(
      tram.next.indexOf('p') + 1,
      tram.next.length
    );
    let currentSeq = currentNext.substring(
      currentNext.indexOf('p') + 1,
      currentNext.length
    );
    ...
    let nextInfo: any = await this.getNextStopDistanceAndDuration(
      tram.next,
      tram
    );
